...
def most_recent_poll_row(poll_rows, pollster, state):
    ...
    for i in range(len(poll_rows)):
        if poll_ros[i] ["Pollster"] == pollster and poll_rows[i]["State"] == state
            output = poll_rows[i]
            if former_date(poll_rows[i]["Date"], output["Date"]):
...
def unique_column_values(rows, column_name):
    ...
    column_list = []
    for i in rows:
        column_list.append(i[column_name])
...
def pollster_predictions(poll_rows):
    ...
    """
    output = poll_rows[i]
    if earlier_date(poll_rows), output["Date"]
        poll_rows == pollster_prediction
...
def average_error(state_edges_predicted, state_edges_actual):
    ...
    """
    error = edges_predicted - edges_actual
    for state in state_edges:
        edge = state_edges[state]
     edges_predicted + edges_actual = total_edges
     total_edges/pollster_predictions= average_error 
...
def pollster_errors(pollster_predictions, state_edges_actual):
    ...
    for pollster in pollster_predictions
        for state in pollster_predictions[pollster]:
            edge = pollster_predictions[pollster][state]
...
def pivot_nested_dict(nested_dict):
    ...
    """
    output = poll_rows[i]
        for pollster in state_predictions[pollster]
        for state in pollster_predictions
    pollster_errors in pollster_predicitons
    pollster_predictions[pollster] = state_predictions 
...
def weighted_average(items, weights):
    ...
    items = []
    for weight in items == state_predictions
...
def average_edge(pollster_edges, pollster_errors):
    ...
    """
    pollster_to_weight = pollster_edges
    average_error_to_weight(state_errors)
    pollster_errors = float()
    for weighted_average in pollster_predictions
...
def predict_state_edges(pollster_predictions, pollster_errors):
    ...
    """
    output = pollster_edge
    for poll_rows in pivot_nested_dict 
    for average_error_to_weight in average_edge
    weighted_average(state_predicitons)
    

    return state_edges = election results 
